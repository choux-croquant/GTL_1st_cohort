...
void SolveAreaConstraintsCS(uint3 DTid : SV_DispatchThreadID)
{
    ...
    // === STEP 1: Compute current triangle area ===
    // Triangle edges from vertex 0
    float3 e1 = p1 - p0;
    float3 e2 = p2 - p0;

    // Compute signed area using rest normal
    // Area = 0.5 * |cross(e1, e2)| but we use signed area with rest normal
    // to detect both expansion and compression
    float3 crossProduct = cross(e1, e2);
    float currentArea = 0.5f * dot(crossProduct, constraint.RestNormal);

    // === STEP 2: Compute constraint violation ===
    float C = currentArea - constraint.RestArea;

    // Early out if constraint is satisfied
    if (abs(C) < EPSILON) return;

    // === STEP 3: Compute constraint gradients ===
    // Gradients derived from area formula: A = 0.5 * dot(cross(e1, e2), n)
    // grad_1 = 0.5 * cross(rest_normal, e2)
    // grad_2 = 0.5 * cross(e1, rest_normal)
    // grad_0 = -(grad_1 + grad_2)  [sum of gradients must be zero]
    
    float3 grad1 = 0.5f * cross(constraint.RestNormal, e2);
    float3 grad2 = 0.5f * cross(e1, constraint.RestNormal);
    float3 grad0 = -grad1 - grad2;

    // === STEP 4: Compute XPBD denominator ===
    // Sum of weighted gradient magnitudes: Σ(w_i * |grad_i|^2)
    ...
    // === STEP 5: XPBD Lagrange multiplier update ===
    ...
    // === STEP 6: Compute position corrections ===
    ...
    // === STEP 7: Atomic accumulation (scaled to int for InterlockedAdd) ===
    ...
    InterlockedAdd(PositionWeight[i2], 1);  // Count constraints
}
