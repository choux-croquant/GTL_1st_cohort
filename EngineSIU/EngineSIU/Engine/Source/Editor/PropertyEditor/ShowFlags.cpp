...
void ShowFlags::Draw(const std::shared_ptr<FEditorViewportClient>& ActiveViewport)
{
    ...
    const char* Items[] = { "AABB", "Primitives", "BillBoardText", "UUID", "Fog", "LightWireframe", "LightWireframeSelectedOnly", "Shadow", "Collision", "CollisionSelectedOnly", "SkeletalMesh", "Particles", "Depth of Field", "Cloth Debug"};
    ...
    if (ImGui::BeginPopup("ShowFlags"))
    {
        bool Selected[IM_ARRAYSIZE(Items)] = 
        {
            ...
            static_cast<bool>(CurFlag & EEngineShowFlags::SF_ClothDebug),
        }; // 각 항목의 체크 상태 저장
...
uint64 ShowFlags::ConvertSelectionToFlags(const bool Selected[])
{
    ...
    if (Selected[13])
    {
        Flags |= static_cast<uint64>(EEngineShowFlags::SF_ClothDebug);
    }
    return Flags;
}
